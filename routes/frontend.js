...
//         if (startsWith.trim() !== '') {
//             url += `&starts_with=${startsWith.toLowerCase()}`;
//         }
//         if (endsWith.trim() !== '') {
//             url += `&ends_with=${endsWith.toLowerCase()}`;
//         }
//         if (contains.trim() !== '') {
//             url += `&contains=${contains.toLowerCase()}`;
//         }
//         if (include.trim() !== '') {
...
//     console.log("req.session.wordfinder", req.session.wordfinder);
...
router.post('/unscramble', visitCounter, async (req, res) => {
    try {
        ...
        if (data && Array.isArray(data.word_pages)) {
            ...
            // Construct redirect URL based on the provided parameters
            let redirectUrl = `/unscramble/${letters}`;
            let queryParams = [];

            if (startsWith.trim() !== '') {
                queryParams.push(`words-start-with/${startsWith.toLowerCase()}`);
            }
            if (endsWith.trim() !== '') {
                queryParams.push(`words-end-in/${endsWith.toLowerCase()}`);
            }
            if (contains.trim() !== '') {
                queryParams.push(`words-contain/${contains.toLowerCase()}`);
            }
            if (length > 0) {
                queryParams.push(`length/${length}`);
            }
            redirectUrl += `/${queryParams.join('/')}/dictionary/${dictionary}`;
...
router.get('/unscramble/:letters/*/dictionary/:dictionary', wrapAsync(async (req, res) => {
    console.log("word_finder_sidebar_form", req.session.wordfinder);

    const { letters, morePosts, startsWith, endsWith, contains, includeLetters, excludeLetters, specifiedLength, wordsByLength } = req.session.wordfinder;
    
    const pathSegments = req.params[0].split('/'); // Get the dynamic segments

    const dynamicParams = {};
    for (let i = 0; i < pathSegments.length; i += 2) {
        dynamicParams[pathSegments[i]] = pathSegments[i + 1];
    }

    res.render('frontend/words-with-X-and-Q.ejs', {
        letters,
        morePosts,
        startsWith: dynamicParams['words-start-with'] || startsWith,
        endsWith: dynamicParams['words-end-in'] || endsWith,
        contains: dynamicParams['words-contain'] || contains,
        includeLetters,
        excludeLetters,
        specifiedLength: dynamicParams['length'] || specifiedLength,
        wordsByLength
    });
}));
...
router.get('/words-that-start-with/:combination', async (req, res) => {
    try {
        ...
        let surroundingLetters = allLetters.slice(Math.max(0, index - 20), Math.min(allLetters.length, index + 40));
        ...
        // Filter surrounding letters to only include those which result in valid words
         const filterLetters = async (letters) => {
            const filteredLettersPromises = letters.map(async (letter) => {
                let url = `https://fly.wordfinderapi.com/api/search?&word_sorting=points&group_by_length=true&page_size=20000&dictionary=${dictionary}&starts_with=${letter}`;
                const response = await fetch(url);
                const data = await response.json();
        ...
        let expandFactor = 1;
        while (filteredLetters.length < 5 && (index - (1 * expandFactor) >= 0 || index + (5 * expandFactor) < allLetters.length)) {
            expandFactor++;
            surroundingLetters = allLetters.slice(Math.max(0, index - (1 * expandFactor)), Math.min(allLetters.length, index + (5 * expandFactor)));
            filteredLetters = await filterLetters(surroundingLetters);
        }
               // Find the index of 'za' in filteredLetters
               console.log("combination",combination);
               const Index = filteredLetters.indexOf(combination);
               console.log("Index",Index);
               // Determine the range to send to the client side
               const startIndex = Math.max(0, Index - 1);
               const endIndex = Math.min(filteredLetters.length, Index + 5); // Include next 5 elements
       
               // Slice the relevant portion of filteredLetters
               const relevantLetters = filteredLetters.slice(startIndex, endIndex);
        ...
        if (data && Array.isArray(data.word_pages)) {
            ...
            res.render('frontend/words-that-start-with.ejs', { letters, morePosts, startsWith: combination, wordsByLength, specifiedLength: length, endsWith, contains, includeLetters: include, excludeLetters: exclude, page, surroundingLetters: relevantLetters ,filter_results:data.filter_results});
        } else {
            console.error('Error: Invalid data structure');
            res.redirect('/no-words-found');
        }
...
router.get('/words-that-end-in/:combination', async (req, res) => {
    try {
        ...
        const filterLetters = async (letters) => {
            const filteredLettersPromises = letters.map(async (letter) => {
                let url = `https://fly.wordfinderapi.com/api/search?&word_sorting=points&group_by_length=true&page_size=20000&dictionary=${dictionary}&ends_with=${letter}`;
                const response = await fetch(url);
                const data = await response.json();
                if (data && data.word_pages && data.word_pages.length > 0) {
                    return letter;
        ...
        if (data && Array.isArray(data.word_pages)) {
            ...
            res.render('frontend/words-that-end-in.ejs', { 
                ...
                page, 
                surroundingLetters: relevantLetters,
...
router.get('/words-with/:contains_char1/and/:contains_char2', async (req, res) => {
    try {
      ...
      const allLetters = 'abcdefghijklmnopqstuvwxyz'.split('');
      ...
      if (contains_char2 === 'z') {
        ...
        const index = allLetters.indexOf(contains_char2);
        let i = index + 1;
        while (surroundingLetters.length < 5 && i < allLetters.length) {
            surroundingLetters.push(allLetters[i]);
            i++;
...
router.get('/words-with/:must_contain/without/:must_not_contain', async (req, res) => {
    try {
        ...
        // Ensure letters are only single characters
        const allLetters = 'abcdefghijklmnopqstuvwxyzabcde'.split('');
        const index = allLetters.indexOf(must_not_contain);
        const surroundingLetters = [];

         // Populate surroundingLetters array
         let i = index + 1;
         while (surroundingLetters.length < 5) {
             let newIndex = i % allLetters.length;
             if (allLetters[newIndex] !== must_contain && !surroundingLetters.includes(allLetters[newIndex])) {
                 surroundingLetters.push(allLetters[newIndex]);
             }
             i++;
         }
...
router.get('/words-that-start-with/:start_with/end-with/:end_with', async (req, res) => {
    try {
        ...
        const morePosts = await Post.find({ status: 'Published' }).limit(3);
         ...
         // Calculate surrounding letter combinations
         const allLetters = 'abcdefghijklmnopqstuvwxyzabcde'.split('');
         const endIndex = allLetters.indexOf(endsWith);
         const startIndex = allLetters.indexOf(startsWith);
         const surroundingCombinations = [];

         // Generate the next 5 combinations
        for (let i = 0; i < 5; i++) {
            let newEndIndex = (endIndex + 1 + i) % allLetters.length;
            let newStartIndex = startIndex;

            if (newEndIndex <= endIndex) {
                newStartIndex = (startIndex + 1) % allLetters.length;
            }

            surroundingCombinations.push({
                start: allLetters[newStartIndex],
                end: allLetters[newEndIndex]
            });
        }
        ...
        if (data && Array.isArray(data.word_pages)) {
            ...
            res.render('frontend/words_that_start_with_x_end_with_y.ejs', { letters, morePosts, startsWith, wordsByLength, specifiedLength: length, endsWith, contains, includeLetters: include, excludeLetters: exclude, page,surroundingCombinations ,filter_results:data.filter_results });
        } else {
            console.error('Error: Invalid data structure');
            res.redirect('/no-words-found');
        }
